...
sirtod_lugares <- sirtod_valid_data |>
	distinct(ubigeo, departamento, provincia, distrito)

sirtod_indicadores <- sirtod_valid_data |>
	distinct(codigoIndicador, arbol, indicador)

sirtod_valores <- sirtod_valid_data |>
	select(codigoIndicador, ubigeo, año, dato)


usethis::use_data(sirtod_lugares, compress = "xz", overwrite = TRUE)
usethis::use_data(sirtod_indicadores, compress = "xz", overwrite = TRUE)
usethis::use_data(sirtod_valores, compress = "xz", overwrite = TRUE)
